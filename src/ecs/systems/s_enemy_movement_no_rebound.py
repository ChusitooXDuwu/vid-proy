...
def system_enemy_movement_no_rebound(world: esper.World, screen_rect: pygame.Rect, enemies_info: dict, total_spawned: int, delta_time: float):
    enemies = []

    for entity, (c_t, c_v, c_s, c_a, _, c_pc) in world.get_components(CTransform, CVelocity, CSurface, CAnimation, CTagEnemy, CPathChange):
    ...
    # Si hay menos de 6 y no hemos llegado a 40, crear más
    while len(enemies) < 6 and total_spawned < 40:
