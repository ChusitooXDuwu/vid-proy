    ...
    def _create_multiple(self, interface_cfg: dict, type: str):
        ...
        for name in interface_cfg[type]:
            if "text" in interface_cfg[type][name]:
                entity_list.append(
                    create_text_interface(
                        self.ecs_world, self.interface_cfg[type][name]
                    )
                )
            elif "image" in interface_cfg[type][name]:
                entity_list.append(
                    create_logo(self.ecs_world, self.interface_cfg[type][name])
                )
        ...
        if c_input.name == "NEXT_SCREEN":
            if c_input.phase == CommandPhase.START:
                if self.mode == "screen_1":
                    ...
                    self.screen_entities = self._create_multiple(
                        self.interface_cfg, "screen_2"
                    )
